 ...
 */
$error_log = isset($_REQUEST['logPath']) ? $_REQUEST['logPath'] : null;
...
function getParam($part, $cursor)
{
	$pRaw = '{' . str_replace("\", ", "\"", $part[$cursor]) . '}';
	$p = json_decode($pRaw);
...
{
	$typeRay = $type;
	$typeRay = str_replace("[", "", $typeRay);
	$typeRay = str_replace("]", "", $typeRay);
	$typeRay = explode(":", $typeRay);
...
function getMsgStr($msg)
{
	$msgRay = $msg;
	$msgRay = preg_replace(['/^"message":"/', '/"$/'], ['', ''], $msgRay);
...
while (!$log->eof()) {
	if (preg_match('/stack trace:$/i', $log->current())) {
		echo "<pre>stack trace</pre>";
		$stackTrace = $parts = [];
		$log->next();

		while (
			(
				preg_match('!^\{(?P<time>[^\}]*)\} PHP\s+(?P<msg>\d+\. .*)$!', $log->current(), $parts) || 
	...
	while (
		!preg_match(
			'!^\{(?P<time>"time":".*",\ )(?P<function>"function":"\[.*\]",\ )(?P<process>"process":"\[.*\]",\ )(?P<msg>"message":".*")\}$!',
			$log->current()
		) && 
		!$log->eof()
	) {
	...
	if (
		...
		$process = explode("\"", @$parts['process']);
		$process = sizeof($process) == 5 ? $process[3] : "[unknown]";
		
		$parts['time'] = getParam($parts, 'time'); //$time->time;
		$parts['function'] = getParam($parts, 'function'); //$function->function;
		$parts['process'] = getParam($parts, 'process'); //$process;
		$parts['type'] = getTypeStr($parts['function']); //$function->function;

		$msg = getMsgStr($parts['msg']);
...
?><!doctype html>
	<head>
		...
		<style type="text/css">
			...
			.fatalerror, .error { border-color: #f26c4f !important; }
	...
	<body>
		<div id="container">
			<?php if (!empty($logs)): ?>
			<p id="serverDetails">
				Error log '<?php echo htmlentities($error_log); ?>' on 
				<?php echo htmlentities($host); ?> (PHP <?php echo phpversion(); ?>, 
			...
			<fieldset id="typeFilter">
				<p>
					...
					<label class="<?php echo $class; ?>">
						...
						<?php echo $title; ?> 
						(<span data-total="<?php echo $typecount[$title]; ?>"><?php echo $typecount[$title]; ?></span>)
					</label>
					<?php endforeach; ?> 
				</p>
			</fieldset>
			...
			<fieldset id="sortOptions">
				<p>
					Sort by: <a href="?type=last&amp;order=asc">last seen (<span>asc</span>)</a>, 
			...
			<section id="errorList">
				...
				<article class="<?php echo $types[$log->type]; ?>"
					...
					<div class="<?php echo $types[$log->type]; ?>">
						<i><?php echo htmlentities($log->type); ?></i> 
						<b><?php echo htmlentities((empty($log->core) ? $log->msg : $log->core)); ?></b><br />
						...
						<p>
							...
							<?php endif; ?> 
							last seen <?php echo date_format(date_create("@{$log->last}"), 'Y-m-d G:ia'); ?>, 
							<?php echo $log->hits; ?> hit<?php echo($log->hits == 1 ? '' : 's'); ?><br />
						</p>
